...
The same rule applies to the Funloom CLI. The published package is `@funloom/cli`, and the command is `funloom`. Treat it as an automation, diagnostics, or advanced configuration tool unless the creator explicitly wants a CLI-first flow.

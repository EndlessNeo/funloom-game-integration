...
- `FUNLOOM_API_BASE_URL` is the Funloom OpenAPI/backend base. `FUNLOOM_FRONTEND_BASE_URL` is the Funloom platform UI base that hosts authorization and recharge pages. It is not the external game's own URL. For standard Funloom environments, the OpenAPI should infer the matching platform frontend and return absolute authorization/recharge URLs from the active API host; do not make creators manually switch this value for ordinary test-vs-production routing. Use explicit frontend/recharge base env only as an override for non-standard, preview, or custom-domain deployments.
...
- Asking users to delete database rows manually instead of designing account recovery, rebind, or conflict-resolution flows.
- Returning vague binding/register failures where the user needs explicit states such as `already_registered`, `already_bound`, `cancelled`, or `rebind_required`.
